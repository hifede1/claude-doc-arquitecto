...
- **Post-release — infraestructura de publicación (#20, 2026-07-18)**: se eliminó el `.claude-plugin/marketplace.json` propio del repo (colisión del nombre `fede-tools` con `claude-audit-tracker`). doc-arquitecto pasa a listarse como fuente `git-subdir` en el catálogo único de `claude-audit-tracker` (`url: https://github.com/hifede1/claude-doc-arquitecto.git`, `path: plugins/doc-arquitecto`, `ref: main` — URL HTTPS completa: el shorthand `owner/repo` se resuelve a SSH y rompe el `install` en máquinas sin SSH configurado). No cambia el plugin (`plugins/doc-arquitecto/`) ni su versión — es infraestructura del repo. Instalación: `/plugin marketplace add hifede1/claude-audit-tracker` → `/plugin install doc-arquitecto@fede-tools`.

- **S05 — Integración y cierre (pipeline e2e)**: la verificación adversarial del criterio 4 (audit-tracker consume la salida sin adaptación manual) encontró dos gaps bloqueantes en la Fase 2 de `/documentar` — el catálogo de `references/` no exponía los `triggers` y `business/` no se cataloga ni llevaba frontmatter machine-readable. **Arreglado en la fuente**: el catálogo ahora cubre AMBAS mitades (references/ = CÓMO, business/ = QUÉ) con todos los campos que el tracker consume (tema · qué resuelve · fecha · triggers con valores · quién la usa · frescura pill 🟢🟠🔴 · path), y `business/` lleva frontmatter `tema`/`fecha`/`triggers`/`resumen`. Re-verificado: pipeline sin traducción confirmado. README final. La publicación (repo público + tag v1.0.0) fue la decisión final de Fede, ejecutada en este release.
