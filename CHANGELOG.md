...
- **Post-release — infraestructura de publicación (#20, 2026-07-18)**: se eliminó el `.claude-plugin/marketplace.json` propio del repo (colisión del nombre `fede-tools` con `claude-audit-tracker`). doc-arquitecto pasa a listarse como fuente `git-subdir` en el catálogo único de `claude-audit-tracker` (`url: hifede1/claude-doc-arquitecto`, `path: plugins/doc-arquitecto`, `ref: main`). No cambia el plugin (`plugins/doc-arquitecto/`) ni su versión — es infraestructura del repo. Instalación: `/plugin marketplace add hifede1/claude-audit-tracker` → `/plugin install doc-arquitecto@fede-tools`.
